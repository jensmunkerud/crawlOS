...
// Smooths out engine movement
	...
	if ((currentMillis - previousMillis >= 10) and (smoothness > 0)) {
		hasUpdated = false;
		if (currentX != xData) {
			currentX = xData > currentX ? min(currentX + smoothness / 100, xData) : max(currentX - smoothness / 100, xData);
			hasUpdated = true;
		}
		if (currentY != yData) {
			currentY = yData > currentY ? min(currentY + smoothness / 100, yData) : max(currentY - smoothness / 100, yData);
			hasUpdated = true;
		}
...
// Parse the received comma-separated data
	...
	if (token != NULL) xData = atoi(token);

	token = strtok(NULL, ",");
	if (token != NULL) yData = atoi(token);
	...
	if (smooth == -1) {
		...
		// Instantly update motors
		currentX = xData;
		currentY = yData;
		updateQuadDrive(xData, yData);
	}
}
